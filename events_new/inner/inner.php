	...
	if(isset($_GET['key']) && !empty($_GET['key']))
	{
		...
		else
		{
			...
			if($result->num_rows==1)
			{
				...
				$main_image = $row['gal_ban'];
				$caption=$row['short_detail'];
				$date = $row['date'];
				$tab = $row['event_name'];
				$description=$row['description'];
				$gimage=$row['gal_img'];
				//$query = "SELECT `gal_img` FROM images WHERE `image_id`='$key' ";
				//$result = $db->query($query);
				$gallery=array();
				if(strlen($gimage)>0)
				{
				$gallery=explode(',',$gimage);
				}
				//$i=0;
				//while($row= $result->fetch_assoc())
				//$gallery[$i++]= $row['gal_img'];
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="data/281FC8A0950474772769C3C2155727A43.css" />
		...
		<div class="bane94_workspace">
			<div class="bane94-halfspan left" >
				...
				<div class="bane94-toolbar">
					<a href="reg.php"><div class="base94-buttons"><input type="submit" value="Register now!" /></div></a>
				</div>
			</div>
...
<div class="regclosebutton" onclick="document.getElementById('register-form').click();"></div>
  ...
  <div style="text-align: center;color: white;font-style: italic;font-size:  14pt;margin-bottom: 235px;">You will need EngiFest ID for entry</div>
...
?>
	...
	<div id="form-container" style="display:none">
		<div id="groupreg" style="width:100%;height:100%;padding-top:20px;">
			...
			<div class="form-sub">
				<div class="form-sub-label" style="width:200px;"><label for="register-name">Enter your Engifest Id(s) :<span style="color:red">*</span></label></div><input type="text" id="register-id" placeholder="Ids separated by semicolon(;)"><br>
				<div id="id-show"></div>
			</div>
			<div class="form-sub">
				<div class="form-sub-label" style="width:200px;">
	...
	<script type="text/javascript" src="s/flowslider.jquery.js"></script>
	<script>
