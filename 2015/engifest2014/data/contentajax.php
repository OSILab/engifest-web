	...
	if(isset($_POST['key']) && !empty($_POST['key']))
	{
		...
		else
		{
			mysql_select_db($db_name,$con);
				...
				$key = $_POST['key'];
				$query = mysql_query("SELECT `tab`,`main_image`, `caption` , `description` , `date` , `images`,`images_thumb` FROM $db_table WHERE `keyword`='$key' ") or die(mysql_error());
	...
	if($_POST['id'] && !empty($_POST['id']) && isset($_POST['email']) && !empty($_POST['email']) && isset($_POST['gevent']) && !empty($_POST['gevent']) && isset($_POST['gname']))
	{
		...
		else
		{
			...
			if(!empty($name) && mysql_num_rows(mysql_query("SELECT `group-name` FROM registrations WHERE `group-name`='$name' AND `event-id`='$eventid'"))>0)
			{
				...
				foreach($ids as $x)
				{
					if($x==$tm)$queryinsert = mysql_query("INSERT INTO registrations (`user-id`,`event-id`,`group-name`,`leader`) VALUES ('$x','$eventid','$name','1')");
					else $queryinsert = mysql_query("INSERT INTO registrations (`user-id`,`event-id`,`group-name`) VALUES ('$x','$eventid','$name')");
				}
				//for registering to group
				//mysql_query("");
				mysql_query("INSERT INTO `groups`(`name`, `event`, `members`, `IDS`, `date_registeration`)
                                 VALUES ('$name','$eventid','".count($ids)."','$id','".time()."')");
	...
	if(isset($_POST['contactname']) && !empty($_POST['contactname']) && isset($_POST['contactemail']) && !empty($_POST['contactemail']) && isset($_POST['contactmessage']) && !empty($_POST['contactmessage']))
	{
				...
				else
				{
					...
					$message = $_POST["contactmessage"];
                                        if(mysql_query("INSERT INTO `contactus`(`name`, `email`, `message`, `time`, `ip`) VALUES ('$name','$email','$message','".time()."','".$_SERVER['REMOTE_ADDR']."')"))
                                        {echo 1;exit;}
